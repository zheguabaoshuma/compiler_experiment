...
AllocaGlobalInstruction::AllocaGlobalInstruction(Operand *dst, SymbolEntry *se, ExprNode* init_expr ,BasicBlock *insert_bb) : Instruction(STORE,insert_bb), init_expr(init_expr)
{
    operands.push_back(dst);
    dst->setDef(this);
    this->se = se;
...
void AllocaGlobalInstruction::output() const
{
    std::string dst, type;
    dst = operands[0]->toStr();
    type = se->getType()->toStr();
    ...
    else if(init_expr->isCTConstant())
